...
export const messages = pgTable("messages", {
  id: varchar("id", { length: 64 }).primaryKey(), // message id from UI
   userId: text("user_id")
    .notNull()
    .references(() => user.id, { onDelete: "cascade" }),
...
export const messageParts = pgTable("message_parts", {
  ...
  messageId: varchar("message_id", { length: 64 })
    ...
    .notNull(),
  type: varchar("type", { length: 20 }).notNull(), // text, file, image, etc.
  text: text("text"), // for text parts
  filename: varchar("filename", { length: 255 }),
  mediaType: varchar("media_type", { length: 100 }),
  url: text("url"), // file or media URL
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at")
    .$onUpdate(() => /* @__PURE__ */ new Date())
    .notNull(),
  extra: jsonb("extra"), // optional metadata
});
